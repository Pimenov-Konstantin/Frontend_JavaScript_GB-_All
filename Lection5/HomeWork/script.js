		...
		const tegEl = document.querySelector('body');
		tegEl.addEventListener('click', function(e) {
		    if (e.target.classList.contains("super_element")) {
		        console.log(`Класс "${e.target.className}" присутствует в элементе "${e.currentTarget.tagName.toLowerCase()}"`);
		    } else {
		...
		// что-то я не понял вопрос... и затрудняюсь ответить...
